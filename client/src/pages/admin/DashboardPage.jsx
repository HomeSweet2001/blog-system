...
export default function DashboardPage() {
  ...
  if (error) {
    return <div className="card p-6 text-sm text-red-600">Erro ao carregar estatisticas: {error}</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-black">Ola, bem-vindo de volta</h1>
          <p className="mt-1 text-sm opacity-70">
            Gerencie o conteudo e a aparencia de <strong>{settings.blog_name}</strong>.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          hint={`${totals.posts_published} publicadas · ${totals.posts_draft} rascunhos`}
        ...
        <StatCard
          ...
          value={
            totals.posts_total ? Math.round(totals.views_total / totals.posts_total) : 0
          }
          hint="Visualizacoes por publicacao"
          color="#10b981"
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <section className="card p-5 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="inline-flex items-center gap-2 font-heading text-lg font-bold">
              <LayoutDashboard className="h-4 w-4 opacity-60" /> Publicacoes recentes
            </h2>
            <Link
              to={paths.posts}
              ...
              {recent.map((post) => (
                <li key={post.id} className="flex items-center gap-3 py-3">
                  <span
                    ...
                    style={{ background: post.category_color || 'var(--c-border)' }}
                  />
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-xs opacity-60">
                      {post.category_name || 'Sem categoria'} · {formatRelative(post.created_at)}
                    </span>
                  </div>
                  <span
        ...
        <section className="card p-5">
          <h2 className="mb-4 font-heading text-lg font-bold">Posts por categoria</h2>
          {byCategory.length === 0 ? (
            ...
            <ul className="space-y-4">
              {byCategory.map((cat) => (
                <li key={cat.name}>
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full" style={{ background: 'var(--c-surface)' }}>
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${(cat.total / maxCategory) * 100}%`, background: cat.color }}
                    />
                  </div>
                </li>
              ))}
